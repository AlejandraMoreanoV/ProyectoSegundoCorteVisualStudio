...
namespace SegundoProyectoVisualStudio
{
    public partial class InsertarSede : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            var request = new RestRequest($"/sede", Method.Post);
            ...
            if (textBox1.Text.Trim() == "" || textBox2.Text.Trim() == ""
                ...
                || textBox5.Text.Trim() == "")
            {
                MessageBox.Show("No ha ingresado todos la información de la sede que intenta guardar.", "Error");
            }
            else if (!int.TryParse(textBox1.Text, out int id))
            {
                MessageBox.Show("El formato del Id no es válido.Debe ser de tipo int");
            }
            else if (int.TryParse(textBox2.Text, out int direccion) || int.TryParse(textBox3.Text, out int ciudad))
            {
                MessageBox.Show("El formato de la direccion y de la ciudad debe ser texto.");
            }
            ...
            {
                MessageBox.Show("El formato del área (m2) no es válido. Debe ser de tipo double.");
            }
            else if (result.StatusCode == HttpStatusCode.BadRequest)
            {
                MessageBox.Show("Error al intentar guardar la sede.", "Error");
            ...
            {
                MessageBox.Show("La sede con estos datos ya existe.", "Error");
                dynamic jsonObj = JsonSerializer.Deserialize<ExpandoObject>(result.Content);
                //textBox6.Text = Convert.ToString(jsonObj.message);
                //MessageBox.Show(jsonObj.message);
            }
            ...
            {
                MessageBox.Show("La sede: " + result.Content + " ha sido agregado exitosamente.");
                textBox1.ResetText();
                textBox2.ResetText();
                textBox3.ResetText();
                textBox4.ResetText();
